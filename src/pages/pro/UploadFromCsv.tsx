...
const UploadFromCsv = () => {
  ...
  const modifyDateFormatToISO = (date: string, oldFormatString: string) => {
    ...
    const formattedDate = newDate.format("YYYY-MM-DDTHH:mm:ssZ");
  ...
  const modifyDataForUpload = (e: any) => {
    ...
    dataFromCSV.forEach((data: any) => {
      const newData: any = {
        recordedAt: modifyDateFormatToISO(
          data[dataFileMapping.recordedAt],
          dateFormat
        ),
        data: {},
        metadata: {
      ...
      dataFileMapping.metrics.forEach((metric: any) => {
        console.log("in mapping",  data[metric], Number(data[metric]),typeof Number(data[metric]), isNaN(Number(data[metric])));
        if (isNaN(Number(data[metric]))){
            data[metric]="";
        }
        newData.data[metric] = Number(data[metric]);
        if (newData.data[metric] === 0 ){
            newData.data[metric] = null;
        }
      });
  ...
  return (
    <div className={css(styles.mainDefault)}>
      <div className={css(styles.dsHeader)}>
        <div className={css(styles.titleHeader)}>Upload Data from CSV</div>
      </div>
      {data.length === 0 && (
        <div className={css(styles.fileInputCont)}>
          <button
            className={css(styles.fileInputBtn)}
